...
const ViewedIndicator: React.FC<ViewedIndicatorProps> = ({ show, className }) => {
  ...
  return (
    <div className={cn(
      "fixed bottom-8 right-8 flex items-center gap-2 bg-green-500/20 backdrop-blur-md py-2 px-4 rounded-full border border-green-500/30 shadow-lg z-50 animate-fade-in",
      className
    )}>
      <CheckCircle2 className="text-green-500 h-5 w-5 animate-pulse" />
      <span className="text-sm font-medium">Termo visualizado</span>
